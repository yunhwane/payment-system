...
data class PaymentExecutionResult (
    ...
    fun paymentStatus(): PaymentStatus {
        return when {
            isSuccess -> return PaymentStatus.SUCCESS
            isFailure -> return PaymentStatus.FAILURE
            isUnknown -> return PaymentStatus.UNKNOWN
            else -> error("결제 (orderId: $orderId) 올바르지 않은 결제 상태입니다.")
        }
    }

    init {
        require(listOf(isSuccess, isFailure, isUnknown).count {it} == 1) {
            "결제 (orderId: $orderId) 올바르지 않은 결제 상태입니다."
        }
    }
}
